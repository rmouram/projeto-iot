...
class EcgClient(fl.client.Client):
    ...
    def get_parameters(self, config) -> GetParametersRes:
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        return FitRes(
            status=Status(code=Code.OK, message="Success"), parameters=params_prime, num_examples=num_examples_train, metrics=metrics
        )
    ...
    def evaluate(self, ins: EvaluateIns) -> EvaluateRes:
        ...
        return EvaluateRes(
            status=Status(code=Code.OK, message="Success"), loss=float(loss), num_examples=len(self.testset), metrics=metrics
        )


def main() -> None:
    """Load data, create and start EcgClient."""
    ...
    trainset, testset = utils.load_ecg(args.cid)

    # Start client
    client = EcgClient(args.cid, model, trainset, testset)
